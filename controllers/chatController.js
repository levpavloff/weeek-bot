...
const updateChatProject = async (ctx, projectId, projectName) => {
    try {
        ...
        const additionalMessage = await ctx.api.sendMessage(groupChatId, `***В этом сообщении мы будем собирать всё самое важное.***

> Это сообщение будет закреплено вверху, чтобы кнопка оставалась на виду.  
> Чтобы закрепить важное сообщение, просто ответьте на него командой /pin. Я добавлю ссылку на это сообщение сюда и кратко опишу его содержание.

***Закрепленные сообщения:***


***Также здесь будут важные статьи*** из нашей базы знаний для вашей работы. Обязательно ознакомьтесь:
`, {
            parse_mode: 'Markdown',
            reply_markup: {
                inline_keyboard: [
