...
const updateChatProject = async (ctx, projectId, projectName) => {
    try {
        ...
        const additionalMessage = await ctx.api.sendMessage(groupChatId, `***А в этом сообщении мы будем собирать всё самое важное***\n\n>Этот пост будет служить закрепом, чтобы сообщение с кнопкой всегда оставалось на виду вверху.\nОтветьте на сообщение, которое хотите закрепить, командой /pin. Я добавлю ссылку на сообщение в этот пост и кратко опишу, о чем добавляемое сообщение.\n\n***Закрепленные сообщения:***\n\n \n\n***Также я закреплю здесь несколько важных статей*** из нашей базы знаний для более эффективной работы. Обязательно почитайте:`, {
            parse_mode: 'Markdown'
        });
